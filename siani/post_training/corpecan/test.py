...
CHECKPOINT_DIR = REPO_ROOT / "outputs" / "qwen_corpecan_lora"
...
DEFAULT_SYSTEM_PROMPT = (
    "You are a virtual assistant trained on CORPECAN interview question-answer pairs. "
    "Reply in natural Canary Islands Spanish when appropriate, preserving oral and regional flavor without inventing facts."
)
...
def main() -> None:
    ...
    if not checkpoint_dir.exists():
        raise FileNotFoundError(
            "Could not find the CORPECAN model checkpoint.\n"
            f"Expected path: {checkpoint_dir}\n"
            "Train it first with siani/post_training/corpecan/train.py."
        )
