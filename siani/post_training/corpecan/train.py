...
DATASET_PATH = REPO_ROOT / "siani" / "data" / "post" / "corpecan_qa_conversations.jsonl"
OUTPUT_DIR = REPO_ROOT / "outputs" / "qwen_corpecan_lora"
...
def train_corpecan(
    *,
    ...
    print("[1/6] Initializing CORPECAN fine-tuning...")
    set_seed(SEED)
...
def resolve_dataset_path(dataset_path: Path | None = None) -> Path:
    ...
    if not resolved_dataset_path.exists():
        raise FileNotFoundError(
            "Could not find the CORPECAN dataset.\n"
            f"Expected path: {resolved_dataset_path}\n"
            "Build it first with siani/data_preparation/build_corpecan_qa_dataset.py."
        )
...
def load_dataset(path: Path) -> tuple[MessageDataset, MessageDataset]:
    ...
    if not train_rows and not eval_rows:
        raise ValueError(
            "No valid examples were found in the CORPECAN dataset.\n"
